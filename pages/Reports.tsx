...
export const Reports: React.FC = () => {
  ...
  // Derived filtered students list based on selected class
  ...
  // Automatically select the first student in the filtered list if the current one is no longer visible
  useEffect(() => {
  ...
  const loadStudentMarks = async () => {
    try {
      ...
      showToast("Failed to load marks for the selected student", "error");
    }
  };
             ...
             <div className="space-y-4">
                <div>
                    ...
                    <div className="relative">
                        <select 
                            className="w-full appearance-none bg-slate-50 border border-slate-300 text-slate-900 py-2 px-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium"
                ...
                <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Filter by Class</label>
                    <div className="relative">
                        <select 
                            className="w-full appearance-none bg-slate-50 border border-slate-300 text-slate-900 py-2 px-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium"
           ...
           {selectedStudentId ? (
                 ...
                 <div className="p-4 sm:p-8">
                    ...
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-8 border-b border-slate-100 pb-6 text-center sm:text-left print:flex-row print:text-left">
                       ...
                       <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-4 w-full">
                           <div>
                               <p className="text-xs text-slate-500 uppercase font-bold tracking-tight">Student Name</p>
                               <p className="font-bold text-slate-800">{selectedStudent?.fullName}</p>
                           </div>
                           <div>
                               <p className="text-xs text-slate-500 uppercase font-bold tracking-tight">Roll Number</p>
                               <p className="font-bold text-slate-800">{selectedStudent?.rollNumber}</p>
                           </div>
                           <div>
                               <p className="text-xs text-slate-500 uppercase font-bold tracking-tight">Class</p>
                               <p className="font-bold text-slate-800">{selectedStudent?.className} - {selectedStudent?.section}</p>
                           </div>
                           <div>
                               <p className="text-xs text-slate-500 uppercase font-bold tracking-tight">Guardian</p>
                    ...
                    <div className="overflow-x-auto mb-8">
                        <table className="w-full min-w-[300px]">
                           ...
                           <tbody className="divide-y divide-slate-50">
                               {Object.keys(consolidatedMarks).length === 0 ? (
                                   <tr>
                                       <td colSpan={6} className="py-12 text-center text-slate-400 italic">No marks recorded for this student in the selected exam.</td>
                                   </tr>
                               ) : (
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap justify-end gap-3 no-print">
                       ...
                       <button 
                           onClick={handlePrint}
                           className="flex items-center gap-2 bg-slate-800 hover:bg-black text-white px-6 py-2 rounded-lg transition-colors justify-center font-medium shadow-md shadow-slate-200"
                       >
                           <Printer size={18} />
                           Print Report
                       </button>
              ...
              <div className="bg-white rounded-xl border border-dashed border-slate-300 py-24 text-center">
                  <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-slate-500 font-medium">Please select a class and student to generate a report preview.</p>
                  </div>
              </div>
           )}
        </div>
