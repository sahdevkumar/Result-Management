  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Teacher's Remark</h1>
            <p className="text-slate-500 text-sm">Add subject-specific performance feedback.</p>
        </div>
        <button 
            onClick={handleSave}
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            <div className="space-y-1">
               <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">1. Select Exam</label>
               <select 
                className="w-full rounded-lg border-slate-300 border p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50 font-bold"
            ...
            <div className="space-y-1">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">2. Select Class</label>
              <select 
                className="w-full rounded-lg border-slate-300 border p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-50 font-bold"
             ...
             <div className="space-y-1">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">3. Subject Access</label>
              <div className="relative">
                  <select 
                    disabled={user.role === 'Teacher'}
                    className={clsx(
                        "w-full rounded-lg border p-2.5 text-sm outline-none font-bold appearance-none",
                        user.role === 'Teacher' ? "bg-slate-100 border-slate-200 text-slate-500" : "bg-slate-50 border-slate-300 text-indigo-700"
                    )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden min-h-[400px]">
         <div className="p-4 bg-slate-50 border-b border-slate-200">
             <h3 className="font-bold text-slate-700 flex items-center gap-2">
                 ...
                 Target: <span className="text-indigo-600">{currentSubject?.name || 'Loading...'}</span>
             </h3>
         </div>
         <div className="overflow-x-auto relative">
            {/* Fixed: Loader2 is now correctly imported */}
            {loading && <div className="absolute inset-0 bg-white/50 backdrop-blur-sm z-10 flex items-center justify-center"><Loader2 className="animate-spin text-indigo-500" /></div>}
                                ...
                                <tr key={student.id} className="group hover:bg-slate-50/50">
                                    <td className="px-6 py-4 align-top">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-slate-400 font-mono uppercase">{student.rollNumber}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-4">
                                        <div className="space-y-2">
                                            <textarea 
                                                className="w-full p-3 border border-slate-300 rounded-xl text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none bg-slate-50 focus:bg-white transition-all text-slate-900"
                                                value={remark}
                                            ...
                                            <div className="flex flex-wrap gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                                                {commonRemarks.map((rem, idx) => (
                                                    <button 
                                                        ...
                                                        className="text-[9px] font-bold bg-white border border-slate-200 px-2.5 py-1 rounded-full text-slate-500 hover:text-indigo-600 hover:border-indigo-300 transition-all shadow-sm"
                                                    >
                                                        {rem}
                                                    </button>
                                                ))}
